...
Becker, Chambers and Wilks which R is an implementation of) by Freidrich Liesch who wrote and maintained the Sweave
package.
...
+ _Settings > Actions > Workflow Permissions > Read and write_
:::
::: {.column width=60%}
...
[✓] Extension installation complete.
Learn more about this extension at https://www.github.com/ArthurData/quarto-confetti
```
...
website:
  title: "SheffieldR - Quarto for Slides"
  site-url: https://ns-rse.github.io/SheffieldR-quarto-for-slides
...
::: {.notes}
Ok, you've now published the slides under your account, but you will want to configure the slides to your own taste or
preferences.

This is done via two files the first is `_quarto.yaml` file which allows you to define the project type, which in this
case is a website and set the `title`, `site-url` and `description`.

However, much of the configuration of how the slides work is done in a second file that contains the content of your
slides the `index.qmd`.
:::
...
::: {.notes}
The `index.qmd` contains the contents of your slides as well as a YAML header which defines  metadata such as Author,
Email and [ORCiD](https://orcid.org). If you haven't got an ORCiD ID yet I'd encourage you to do so.
...
I'm not going to go into great detail about the syntax of Markdown because the template you have clone includes sample
slides with different content and formats such as using columns, embedding pictures and links, ordered and unordered
lists and so forth. Copy and paste the sections you want to use and change.

Instead I'll highlight a few key features and how to work with the slides and get them published on GitHub after having
change them, otherwise what is the point of using this template!.
:::
...
::: {.notes}
For the GitHub Action the theme and extension that are included in the template are already there, but if you add any
extensions you should add them to the section that Installs Quarto Extensions
:::
...
If the chalkboard feature wasn't enough nostalgia you can use the [Blackboard
Theme](https://github.com/schochastics/quarto-blackboard-theme).
...
::: {.notes}
Quarto will render your slides locally using `quarto preview` command which makes the HTML files from your source
markdown and start a web-server for you to view the pages _before_ you push them to GitHub.
...
## GitHub Actions - Software Dependencies
...
Make sure you stage and commit this file to your git repository so that it gets uploaded to GitHub when you push your
changes.
:::
